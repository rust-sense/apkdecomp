...
public final class SentryClient implements ISentryClient, IMetricsClient {
    ...
    public SentryId captureEvent(SentryEvent sentryEvent, IScope iScope, Hint hint) {
        ...
        if (sentryEvent3 != null && (throwable = sentryEvent.getThrowable()) != null && this.options.containsIgnoredExceptionForType(throwable)) {
        ...
        if (shouldApplyScopeData(sentryEvent, hint2) && (sentryEvent3 = applyScope(sentryEvent, iScope, hint2)) == null) {
            this.options.getLogger().log(SentryLevel.DEBUG, "Event was dropped by applyScope", new Object[0]);
            return SentryId.EMPTY_ID;
        }
        SentryEvent processEvent = processEvent(sentryEvent3, hint2, this.options.getEventProcessors());
        ...
        if (processEvent != null) {
            Session updateSessionData = (withSession == null || !withSession.isTerminated()) ? updateSessionData(processEvent, hint2, iScope) : null;
            if (sample()) {
        ...
        try {
            ...
            SentryEnvelope buildEnvelope = buildEnvelope(sentryEvent2, sentryEvent2 != null ? getAttachments(hint2) : null, session, traceContext2, null);
            hint2.clear();
            if (buildEnvelope != null) {
                sentryId2 = sendEnvelope(buildEnvelope, hint2);
            }
        } catch (SentryEnvelopeException | IOException e) {
            this.options.getLogger().log(SentryLevel.WARNING, e, "Capturing event %s failed.", sentryId2);
            sentryId2 = SentryId.EMPTY_ID;
        }
        if (iScope != null && (transaction = iScope.getTransaction()) != null && HintUtils.hasType(hint2, TransactionEnd.class)) {
            Object sentrySdkHint = HintUtils.getSentrySdkHint(hint2);
            if (sentrySdkHint instanceof DiskFlushNotification) {
                ((DiskFlushNotification) sentrySdkHint).setFlushable(transaction.getEventId());
                transaction.forceFinish(SpanStatus.ABORTED, false, hint2);
            } else {
            ...
            if (iScope != null) {
                return iScope.withSession(new Scope.IWithSession() { // from class: io.sentry.SentryClient$$ExternalSyntheticLambda1
                    ...
                    public final void accept(Session session) {
                        SentryClient.this.m773lambda$updateSessionData$1$iosentrySentryClient(sentryEvent, hint, session);
                    }
                });
            }
    ...
    /* synthetic */ void m773lambda$updateSessionData$1$iosentrySentryClient(SentryEvent sentryEvent, Hint hint, Session session) {
        if (session == null) {
