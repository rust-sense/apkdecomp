...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m2480constructorimpl = SegmentOrClosed.m2480constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m2480constructorimpl = SegmentOrClosed.m2480constructorimpl(segment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m2485isClosedimpl(m2480constructorimpl)) {
                Segment m2483getSegmentimpl = SegmentOrClosed.m2483getSegmentimpl(m2480constructorimpl);
                while (true) {
                    Segment segment4 = (Segment) this.tail;
                    if (segment4.getId() >= m2483getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!m2483getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment4, m2483getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m2483getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m2483getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m2483getSegmentimpl(m2480constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment4, m2483getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m2483getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
